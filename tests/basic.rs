...
#[test]
fn test_withdraw_released_reserved() {
    let (e, tokens, users) = basic_setup();
    ...
    new_config.release_ratio = 10000;
    ...
    assert!(format!("{:?}", e.withdraw_reserved(&tokens.ndai, Some(1.into()))
        .promise_errors()[0].as_ref().unwrap().status()).contains("Too much to withdraw at once!"));

    new_config.max_utilization_impact_rate = 1000;
    e.update_asset(&tokens.ndai, new_config.clone());

    e.withdraw_released(&tokens.ndai, Some(10000.into())).assert_success();
    ...
    assert_eq!(asset.released - 10000, asset_after_withdraw.released);

    let current_balance = e.ft_balance_of(&tokens.ndai, &e.owner).0;
    
    assert_eq!(20000, current_balance - old_balance);

    assert!(format!("{:?}", e.withdraw_released(&tokens.ndai, Some((asset_after_withdraw.released * 2).into()))
        .promise_errors()[0].as_ref().unwrap().status()).contains("Released balance not enough!"));
    ...
    assert_eq!(e.get_asset(&tokens.ndai).released, asset_after_withdraw.released);
    ...
    assert_eq!(asset.released, asset_after_withdraw.released);
    ...
    assert!(format!("{:?}", e.withdraw_released(&tokens.ndai, Some(10000.into()))
        .promise_errors()[0].as_ref().unwrap().status()).contains("The account owner.near is not registered"));
