...
fn parse_token_id(token_id: String) -> TokenOrPool {
    if let Ok(pool_id) = try_identify_pool_id(&token_id) {
        ...
        TokenOrPool::Token(AccountId::try_from(token_id).unwrap())
    }
}
...
impl Contract {
    ...
    pub fn mft_has_registered(&self, token_id: String, account_id: AccountId) -> bool {
        match parse_token_id(token_id) {
            TokenOrPool::Token(_) => false,
            TokenOrPool::Pool(pool_id) => {
                if let Some(pool) = self.pools.get(pool_id) {
                    pool.share_has_registered(&account_id)
                } else {
                    false
                }
            }
    ...
    pub fn mft_register(&mut self, token_id: String, account_id: AccountId) {
        ...
        match parse_token_id(token_id) {
            TokenOrPool::Token(_) => env::panic_str(ERR110_INVALID_REGISTER),
            TokenOrPool::Pool(pool_id) => {
                let mut pool = self.pools.get(pool_id).expect(ERR85_NO_POOL);
