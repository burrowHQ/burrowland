...
pub const GAS_FOR_DEPOSIT_FREE_SHADOW_SEED: Gas = Gas(Gas::ONE_TERA.0 * 50);
pub const GAS_FOR_DEPOSIT_FREE_SHADOW_SEED_CALLBACK: Gas = Gas(Gas::ONE_TERA.0 * 20);

pub const GAS_FOR_WITHDRAW_FREE_SHADOW_SEED: Gas = Gas(Gas::ONE_TERA.0 * 50);
pub const GAS_FOR_WITHDRAW_FREE_SHADOW_SEED_CALLBACK: Gas = Gas(Gas::ONE_TERA.0 * 20);

pub const GAS_FOR_BURROWLAND_DEPOSIT_SHADOW_ASSET: Gas = Gas(Gas::ONE_TERA.0 * 50);
pub const GAS_FOR_BURROWLAND_DEPOSIT_SHADOW_ASSET_CALLBACK: Gas = Gas(Gas::ONE_TERA.0 * 20);
...
impl Contract {
    ...
    pub fn sync_lp_infos(
        &self,
        pool_ids: Vec<u64>,
    ) -> HashMap<String, UnitShareTokens> {
    ...
    pub fn shadow_farming(&mut self, pool_id: u64, amount: Option<U128>) -> PromiseOrValue<bool> {
        ...
        ext_boost_farm_receiver::ext(self.boost_farm_id.clone())
            .with_static_gas(GAS_FOR_DEPOSIT_FREE_SHADOW_SEED)
            .deposit_free_shadow_seed(
                sender_id.clone(),
                seed_id,
                U128(amount)
            )
            .then(
                Self::ext(env::current_account_id())
                    .with_static_gas(GAS_FOR_DEPOSIT_FREE_SHADOW_SEED_CALLBACK)
                    .callback_deposit_free_shadow_seed(
                        sender_id,
                        pool_id,
                        U128(amount),
                        U128(storage_cost)
    ...
    pub fn shadow_cancel_farming(&mut self, pool_id: u64, amount: Option<U128>) -> PromiseOrValue<bool> {
        ...
        ext_boost_farm_receiver::ext(self.boost_farm_id.clone())
            ...
            .then(
                Self::ext(env::current_account_id())
                    .with_static_gas(GAS_FOR_WITHDRAW_FREE_SHADOW_SEED_CALLBACK)
                    .callback_withdraw_free_shadow_seed(
                        sender_id,
                        pool_id,
                        U128(amount),
                        U128(storage_refund)
    ...
    pub fn shadow_burrowland_deposit(&mut self, pool_id: u64, amount: Option<U128>, after_deposit_actions_msg: Option<String>) -> PromiseOrValue<bool> {
        ...
        .deposit_shadow_asset(
            sender_id.clone(),
            AccountId::new_unchecked(token_id),
            U128(amount),
            after_deposit_actions_msg
        )
        .then(
            Self::ext(env::current_account_id())
                .with_static_gas(GAS_FOR_BURROWLAND_DEPOSIT_SHADOW_ASSET_CALLBACK)
                .callback_deposit_shadow_asset(
                    sender_id,
                    pool_id,
                    U128(amount),
                    U128(storage_cost)
    ...
    pub fn shadow_burrowland_withdraw(&mut self, pool_id: u64, amount: Option<U128>, before_withdraw_actions_msg: Option<String>) -> PromiseOrValue<bool> {
        ...
        ext_burrowlnad_receiver::ext(self.burrowland_id.clone())
            ...
            .then(
                Self::ext(env::current_account_id())
                    .with_static_gas(GAS_FOR_DEPOSIT_FREE_SHADOW_SEED_CALLBACK)
                    .callback_withdraw_shadow_asset(
                        sender_id,
                        pool_id,
                        U128(amount),
                        U128(storage_refund)
...
impl Contract {
    ...
    pub fn callback_deposit_free_shadow_seed(
        ...
        if !is_promise_success() {
            // The account cannot be storage_unregister when the account has a shadow record
            let mut account = self.internal_unwrap_account(&sender_id); 
