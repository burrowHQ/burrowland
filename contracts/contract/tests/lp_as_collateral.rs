...
#[tokio::test]
async fn test_asset_config_min_shares() -> Result<()> {
    let worker = workspaces::sandbox().await?;
    let root = worker.root_account()?;
    ...
    let burrowland_contract = deploy_burrowland(&root).await?;
    {
        ...
        check!(burrowland_contract.add_asset(&root, &token_id, AssetConfig{
            ...
            can_borrow: false,
            net_tvl_multiplier: 10000,
            min_reserve_shares: d(10,18).into()
        }));
        check!(wrap_token_contract.ft_mint(&root, &root, parse_near!("10000 N")));
        check!(burrowland_contract.deposit_to_reserve(&wrap_token_contract, &root, parse_near!("10000 N")));
        check!(burrowland_contract.storage_deposit(&root));
    ...
    check!(ref_exchange_contract.shadow_burrowland_deposit(&alice, 0, Some(d(2,18).into()), None), "Asset shadow_ref_v1-0 supply shares cannot be less than 10000000000000000000");
    check!(burrowland_contract.update_asset_config_min_reserve_shares(&root, &token_id, d(20,18)));
    check!(ref_exchange_contract.shadow_burrowland_deposit(&alice, 0, Some(d(2,18).into()), None), "Asset shadow_ref_v1-0 supply shares cannot be less than 20000000000000000000");
    check!(ref_exchange_contract.shadow_burrowland_deposit(&alice, 0, Some(d(20,18).into()), None));

    check!(burrowland_contract.sync_ref_exchange_lp_token_infos(&root, Some(vec![token_id.to_string().clone()])));
    check!(burrowland_contract.position_increase_collateral(&alice, &token_id, 0));
    let current_timestamp = worker.view_block().await?.timestamp();
    check!(burrowland_contract.update_asset_config_min_reserve_shares(&root, wrap_token_contract.0.id(), d(5,24)));
    check!(burrowland_contract.position_borrow_and_withdraw(&alice, &oralce_contract, burrowland_contract.0.id(), 
    price_data(current_timestamp, Some(100000)), token_id.to_string(), wrap_token_contract.0.id(), parse_near!("1 N"), 0), "Asset wrap.test.near supply shares cannot be less than 5000000000000000000000000");
    check!(burrowland_contract.position_borrow_and_withdraw(&alice, &oralce_contract, burrowland_contract.0.id(), 
    price_data(current_timestamp, Some(1000)), token_id.to_string(), wrap_token_contract.0.id(), parse_near!("5 N"), 0));
    check!(view burrowland_contract.get_account_all_positions(&alice));
