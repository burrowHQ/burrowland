...
impl MarginTradingPosition {
    fn new(open_ts: Timestamp, margin_asset: TokenId, margin_shares: Shares, debt_asset: TokenId, position_asset: TokenId) -> Self {
...
impl Contract {
    pub(crate) fn internal_margin_open_position(
        ...
        assert!(!account.margin_positions.contains_key(&pos_id), "Margin position already exist");
        ...
        if min_position_amount < et_position_amount {
            assert!(min_position_amount >= et_position_amount * tbd_slippage_rate / MAX_RATIO as u128, "min_position_amount is too low");
        }
        ...
        assert!(total_cap.mul_ratio(tbd_safty_buffer) + total_cap > total_debt, "Debt is too much");
        ...
        ext_fungible_token::ext(debt_id.clone())
            ...
            .ft_transfer_call(
                ...
                Self::ext(env::current_account_id())
                    ...
                    .callback_dex_trade(account.account_id.clone(), pos_id.clone(), debt_amount.into(), U128(0), format!("open"))
            );
    ...
    pub(crate) fn internal_margin_close_position(
        ...
        let mut mt = account.margin_positions.get_mut(pos_id).expect("Position not exist");
        ...
        //   mt is running
        assert!(!mt.is_locking, "Position is currently waiting for trading result.");
        ...
        if min_debt_amount < et_debt_amount {
            assert!(min_debt_amount >= et_debt_amount * tbd_slippage_rate / MAX_RATIO as u128, "min_position_amount is too low");
        }
        //   ensure enough position token to trade
        if position_amount > mt.position_amount {
            // try to add some of margin asset into trading
            assert_eq!(mt.margin_asset, mt.position_asset, "Not enough position asset balance");
            let gap_shares = asset_pos.supplied.amount_to_shares(position_amount - mt.position_amount, true);
            mt.margin_shares.0.checked_sub(gap_shares.0).expect("Not enough position asset balance");
            asset_pos.supplied.withdraw(gap_shares, position_amount - mt.position_amount);
        ...
        let total_debt_amount = asset_debt.margin_debt.shares_to_amount(mt.debt_shares, true);
        if min_debt_amount < total_debt_amount {
            assert_eq!(mt.margin_asset, mt.debt_asset, "Can NOT trade under total debt when margin and debt asset are not the same");
            let gap_shares = asset_debt.supplied.amount_to_shares(total_debt_amount - min_debt_amount, true);
            assert!(mt.margin_shares.0 > gap_shares.0, "Not all debt could be repaid");
        }
        ...
        ext_fungible_token::ext(position_id.clone())
            ...
            .ft_transfer_call(
                ...
                Self::ext(env::current_account_id())
                    .with_static_gas(GAS_FOR_FT_TRANSFER_CALL_CALLBACK)
                    .callback_dex_trade(account.account_id.clone(), 
                    pos_id.clone(), position_amount.into(), pre_position_amount.into(), format!("decrease"))
            );
    }
    ...
    pub(crate) fn internal_margin_liquidate_position(
        ...
        let mut mt = pos_owner.margin_positions.get_mut(pos_id).expect("Position not exist");
        let pre_position_amount = mt.position_amount;
        ...
        let total_debt = self.get_mtp_borrowed_sum(&mt, prices);
        let tbd_safty_buffer = 1000_u32;
        assert!(total_cap.mul_ratio(tbd_safty_buffer) + total_cap < total_debt, "Margin position is not liquidatable");
        //   min_debt_amount reasonable
        let position_value = BigDecimal::from_balance_price(
            position_amount,
            prices.get_unwrap(&mt.position_asset),
        ...
        ext_fungible_token::ext(position_id.clone())
            ...
            .ft_transfer_call(
                ...
                Self::ext(env::current_account_id())
                    .with_static_gas(GAS_FOR_FT_TRANSFER_CALL_CALLBACK)
                    .callback_dex_trade(pos_owner.account_id.clone(), 
                    pos_id.clone(), position_amount.into(), pre_position_amount.into(), format!("decrease"))
            );
    }
}
...
impl Contract {
    ...
    #[private]
    pub fn callback_dex_trade(&mut self, account_id: AccountId, pos_id: PosId, amount_in: U128, pre_position_amount: U128, op: String) {
        let amount_in_used = if let Some(cross_call_result) = promise_result_as_success() {
            serde_json::from_slice::<U128>(&cross_call_result).unwrap().0
        } else {
            0_u128
        };
        if amount_in_used == 0 {
